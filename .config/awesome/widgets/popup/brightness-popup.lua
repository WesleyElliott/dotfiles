...
screen.connect_signal("request::desktop_decoration", function(screen)
    ...
    screen.brightness_popup = awful.popup {
        widget = {},
        ontop = true,
        visible = false,
        type = "notification",
...
local popup_placer = function()
    local focused = awful.screen.focused()
    local popup = focused.brightness_popup
...
awesome.connect_signal("evil::brightness", function(volume)
    if volume.show_popup then
        awful.screen.focused().brightness_popup.visible = true
